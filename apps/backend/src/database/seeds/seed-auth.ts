 ...
 *
 * Usage:
 * - Run seeder (append): npm run seed:auth
 * - Run seeder (clean & seed): npm run seed:auth:clean
 *
 * Alternative (inside Docker):
 * - docker compose -f docker-compose.dev.yml exec backend npm run seed:auth
 *
 * Safety features:
...
async function confirmProductionClean(nodeEnv: string): Promise<boolean> {
  ...
  console.log('\n⚠️  ⚠️  ⚠️  WARNING ⚠️  ⚠️  ⚠️');
  console.log('You are about to DELETE all users in PRODUCTION!');
  ...
  return new Promise((resolve) => {
    rl.question('Type "DELETE ALL USERS" to confirm: ', (answer) => {
      rl.close();
